...
static int __do_proc_dointvec(void *tbl_data, struct ctl_table *table,
  ...
  for (; left && vleft--; i++, first = 0) {
    ...
    if (write) {
      ...
      if (!left)
        break;
      err = proc_get_long(&kbuf, &left, &lval, &neg, proc_wspace_sep,
...
static int __do_proc_doulongvec_minmax(void *data, struct ctl_table *table,
  ...
  for (; left && vleft--; i++, first = 0) {
    ...
    if (write) {
      ...
      err = proc_get_long(&kbuf, &left, &val, &neg, proc_wspace_sep,
                          sizeof(proc_wspace_sep), NULL);
      if (err)
        break;
      if (neg)
...
int proc_do_large_bitmap(struct ctl_table *table, int write,
  ...
  if (write) {
    ...
    while (!err && left) {
      ...
      err = proc_get_long(&kbuf, &left, &val_a, &neg, tr_a, sizeof(tr_a), &c);
      if (err)
        break;
      ...
      if (c == '-') {
        err = proc_get_long(&kbuf, &left, &val_b, &neg, tr_b, sizeof(tr_b), &c);
        if (err)
          break;
